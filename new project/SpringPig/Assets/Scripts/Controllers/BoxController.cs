...
public class BoxController : MonoBehaviour
{
    ...
    private void PushBox(PlayerController playerObject)
    {
        ...
        if (playerObject.IsMovingHorizontally())
        {
            ...
            if (isCollisionX && player.velocity.x != 0 && MathUtils.IsSameSign(player.velocity.x, collisionDirection.x))
            {
            ...
            else if (!isCollisionX && player.velocity.z != 0 && MathUtils.IsSameSign(player.velocity.z, collisionDirection.z))
            {
